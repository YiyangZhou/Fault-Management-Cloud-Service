...
from utils.native_bayes import NaiveBayes
    ...
    def __iter__(self):
        for item in self.data_list:
            ...
            if param_url:
                ...
                url = self.request.path_info
            name_list = ['新建', '处理中', '已解决', '已忽略', '待反馈', '已关闭', '重新打开']
            #####
            # # 获取各个问题状态数量
            # num_list = [0, 0, 0, 0, 0, 0, 0]
            # ret = models.Issues.objects.all().annotate(c=Count('status')).values('status', 'c').first()
            # num_list[ret['status'] - 1] = ret['c']
...
def issues(request, project_id):
    ...
    ########################
    # 朴素贝叶斯+tf-idf问题分类
    nb = NaiveBayes(request.POST.get('subject'))
    issues_type = nb.predict()
    ...
    if form.is_valid():
        ...
        if not models.IssuesType.objects.filter(title=issues_type, project_id=request.tracer.project.id).first():
            models.IssuesType.objects.create(title=issues_type, project_id=request.tracer.project.id)
        form.instance.issues_type = models.IssuesType.objects.filter(title=issues_type, project_id=request.tracer.project.id).first()
        form.save()
